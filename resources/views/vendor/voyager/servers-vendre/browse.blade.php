            ...
            <div class="card">
                ...
                <div class="d-flex align-items-start justify-content-between">
                    ...
                    <div class="d-flex align-items-center my-2">
                        <h3 class="mb-0 me-2">{{ \App\Models\OfferServer::count() }}</h3>
                    </div>
                    ...
                    <div class="col user_role">
                        <select id="search_key" name="key" class="form-select text-capitalize">
                            @foreach($searchNames as $key => $name)
                                @if ( in_array($name, ['Image', 'Map Id', 'Map']) == false )
                                    ...
                                    @elseif($row->type == 'file' && !empty($data->{$row->field}) )
                                        ...
                                        @if(json_decode($data->{$row->field}) !== null)
                                            @foreach(json_decode($data->{$row->field}) as $file)
                                                ...
                                                @if ( pathinfo( $file->original_name, PATHINFO_EXTENSION) == 'svg'  )
                                                    <div class="avatar-wrapper">
                                                        <div class="avatar me-3">
                                                            <img src="@if( !filter_var($file->download_link, FILTER_VALIDATE_URL)){{ Voyager::image( $file->download_link ) }}@else{{ $file->download_link }}@endif" class="rounded-circle">
    ...
    <div class="offcanvas offcanvas-top" tabindex="-1" id="deleteConvas" aria-labelledby="deleteConvasLabel">
        ...
        <div class="offcanvas-body">
            <form action="#"  id="delete_form" method="POST">
                {{ method_field('DELETE') }}
                {{ csrf_field() }}
                <button type="submit"  class="btn btn-danger me-2 delete-confirm">Continue</button>
